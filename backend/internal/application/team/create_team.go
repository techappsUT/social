...
func (uc *CreateTeamUseCase) Execute(ctx context.Context, input CreateTeamInput) (*CreateTeamOutput, error) {
	...
	// Create a slug existence checker function
	slugExists := func(slug string) bool {
		exists, err := uc.teamRepo.ExistsBySlug(ctx, slug)
		if err != nil {
			uc.logger.Warn("Failed to check slug existence", "slug", slug, "error", err)
	...
	uniqueSlug := generateUniqueSlug(baseSlug, slugExists)

	uc.logger.Info("Generated unique slug", "baseSlug", baseSlug, "uniqueSlug", uniqueSlug)

	// 3. Create team entity
	newTeam, err := team.NewTeam(input.Name, uniqueSlug, "", input.OwnerID)
	if err != nil {
		return nil, fmt.Errorf("invalid team data: %w", err)
	}
	...
	uc.logger.Info("Team created successfully", "teamId", newTeam.ID(), "slug", uniqueSlug, "ownerId", input.OwnerID)
